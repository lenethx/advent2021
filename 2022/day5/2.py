...
with open("input.txt") as file:
    ...
    for item in movedata:
        parts=item.split(" ")
        piles[int(parts[5])-1]=piles[int(parts[5])-1]+piles[int(parts[3])-1][-int(parts[1]):]
        piles[int(parts[3])-1]=piles[int(parts[3])-1][:-int(parts[1])]

    print(''.join((l2(piles).map(lambda x: x[-1]))))
