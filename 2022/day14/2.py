...
sandmat=[[" " for x in range(min(xvals), max(xvals)+401)]  for y in range(0, max(yvals)+3)  ]
    ...
    for i in range(0, len(rockcoords)-1):
        ...
        if coord1[0]==coord2[0]:
            for y in range(min(coord1[1], coord2[1]),max(coord1[1], coord2[1])+1):
                sandmat[y][coord1[0]-min(xvals)+200]="#"
        else:
            for x in range(min(coord1[0], coord2[0]),max(coord1[0], coord2[0])+1):
                sandmat[coord1[1]][x-min(xvals)+200]="#"

for x in range(0, len(sandmat[-1])):
    sandmat[-1][x]='#'
