...
with open("input.txt") as file:
    lines=list(map(lambda x :list(map(lambda y: list(map(lambda z:list(map(lambda w:list(map(intifint,w.split('..'))),z.split('='))),y.split(','))),x[:-1].split(' '))),file.readlines()[::-1]))
    lines=list(map(lambda x : [x[0][0][0][0], [x[1][0][1][0],x[1][0][1][1]],[x[1][1][1][0],x[1][1][1][1]],[x[1][2][1][0],x[1][2][1][1]] ] ,lines))
    
    
    

# xmat={x:0 for x in range(-100000,100000)}

# mat={z:{y: xmat.copy() for y in range(-100000,100000)} for z in range(-100000,100000)}
# prmat(lines)
# prmat(mat)
